...
public class Arguments {
  //
  private long numberOfHands = Constants.NUMBER_OF_HANDS_DEFAULT;
  private long numberOfThreads = Constants.NUMBER_OF_CORES_DEFAULT;
    ...
    for (int i = 0; i < args.length; i++) {
      ...
      if ((arg.equals("-h") || arg.equals("--number-of-hands")) && i + 1 < args.length) {
        try {
          ...
          if (numberOfHands < Constants.NUMBER_OF_HANDS_MINIMUM
              || numberOfHands > Constants.NUMBER_OF_HANDS_MAXIMUM) {
            throw new IllegalArgumentException(
                "Number of hands must be between "
                    + Constants.NUMBER_OF_HANDS_MINIMUM
                    + " and "
                    + Constants.NUMBER_OF_HANDS_MAXIMUM);
          }
  ...
  private void printHelpMessage() {
    System.out.println(
        "Usage: strikerJava [options]\n"
            ...
            + "  -h, --number-of-hands <number of hands>  The number of hands to play in this"
            + " simulation\n"
            + "  -M, --mimic                              Use the mimic dealer player strategy\n"
            + "  -B, --basic                              Use the basic player strategy\n"
            + "  -N, --neural                             Use the neural player strategy\n"
            + "  -L, --linear                             Use the liner regression player"
            + " strategy\n"
            + "  -P, --polynomial                         Use the polynomial regression player"
            + " strategy\n"
