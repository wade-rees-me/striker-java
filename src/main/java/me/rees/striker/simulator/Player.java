...
public class Player {
  ...
  public Player(Rules rules, Strategy strategy, int numberOfCards) {
    ...
    this.seenCards =
        new int[Shoe.MAXIMUM_CARD_VALUE + 1]; // Keeps track of the cards the player has seen
  }
  ...
  // Payoff for the hand
  private void payoffHand(
      Wager wager, boolean dealerBlackjack, boolean dealerBusted, int dealerTotal) {
    if (dealerBlackjack) {
      wager.wonInsurance();
      if (wager.isBlackjack()) {
        wager.push();
